  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
        {/* Scores Column */}
        <div className="col-span-1 flex flex-col sm:flex-row md:flex-col gap-4 md:gap-6">
          {/* Safety Score Card */}
          <div className={`rounded-3xl border p-5 md:p-6 flex flex-col items-center justify-center text-center flex-1 ${getScoreBg(result.safetyScore)}`}>
            <p className="text-xs md:text-sm font-semibold uppercase tracking-wider opacity-70 mb-2">{t.safetyScore}</p>
            <div className={`text-4xl md:text-5xl font-bold mb-2 ${getScoreColor(result.safetyScore)}`}>
              {result.safetyScore}
              <span className="text-lg md:text-xl opacity-50">/100</span>
            </div>
          ...
          {/* Halal Score Card */}
          <div className={`rounded-3xl border p-5 md:p-6 flex flex-col items-center justify-center text-center flex-1 ${getHalalBg(result.halalStatus)}`}>
            <p className="text-xs md:text-sm font-semibold uppercase tracking-wider opacity-70 mb-2">{t.halalScore}</p>
            <div className={`text-4xl md:text-5xl font-bold mb-3 ${getHalalColor(result.halalStatus)}`}>
              {result.halalScore}
              <span className="text-lg md:text-xl opacity-50">/100</span>
            </div>
            <div className={`flex items-center gap-2 px-3 md:px-4 py-1.5 md:py-2 rounded-full font-bold uppercase tracking-wide text-[10px] md:text-sm shadow-sm ${
              ...
              {result.halalStatus === 'halal' && (
                <div className="bg-emerald-700 text-white text-[8px] md:text-[10px] px-1.5 py-0.5 rounded leading-none">حلال</div>
              )}
              {result.halalStatus === 'doubtful' && <HelpCircle className="w-3 h-3 md:w-4 md:h-4" />}
              {result.halalStatus === 'haram' && <XCircle className="w-3 h-3 md:w-4 md:h-4" />}
        ...
        {/* Summary Card */}
        <div className="col-span-1 md:col-span-2 bg-white rounded-3xl border border-emerald-100 p-6 md:p-8 shadow-sm shadow-emerald-900/5">
          <h3 className="text-lg md:text-xl font-display font-semibold text-black mb-4 flex items-center gap-2">
          ...
          <div className="bg-emerald-50 rounded-2xl p-4 md:p-5 border border-emerald-100">
            <h4 className="text-xs font-semibold uppercase tracking-wider text-emerald-800 mb-2">{t.recommendation}</h4>
      ...
      <div className="bg-white rounded-3xl border border-emerald-100 p-6 md:p-8 shadow-sm shadow-emerald-900/5">
        <h3 className="text-xl md:text-2xl font-display font-semibold text-black mb-6 md:mb-8">{t.breakdown}</h3>
        
        <div className="space-y-6 md:space-y-8">
          {/* Avoid */}
          {avoidIngredients.length > 0 && (
            <div>
              <h4 className="flex items-center gap-2 text-rose-700 font-display font-semibold mb-4 text-lg border-b border-rose-100 pb-2">
          ...
          {cautionIngredients.length > 0 && (
            <div>
              <h4 className="flex items-center gap-2 text-amber-700 font-display font-semibold mb-4 text-lg border-b border-amber-100 pb-2">
          ...
          {safeIngredients.length > 0 && (
            <div>
              <h4 className="flex items-center gap-2 text-emerald-700 font-display font-semibold mb-4 text-lg border-b border-emerald-100 pb-2">
      ...
      {/* Actions */}
      <div className="flex flex-col sm:flex-row justify-center gap-3 md:gap-4 pt-4 pb-12 px-4 sm:px-0">
        <button
          onClick={onReset}
          className="flex items-center justify-center gap-2 bg-emerald-100 hover:bg-emerald-200 text-emerald-800 px-6 py-3.5 rounded-xl font-medium transition-colors w-full sm:w-auto"
        >
